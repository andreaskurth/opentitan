...
filesets:
  files_rtl:
    depend:
      ...
      # prim_flash is a macro, so it is not part of a library and must be
      # named explicitly.
      - lowrisc:prim:flash
...
To avoid repeating them, put them in a `dvsim.hjson` file, which `dvsim` finds by walking up from the working directory:
