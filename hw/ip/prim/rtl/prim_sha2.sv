...
#(
  ...
  assign digest_mode_flag_d = hash_start  ? digest_mode_i   :    // latch in configured mode
  ...
  if (MultimodeEn) begin : gen_multimode
    ...
    always_comb begin : compute_w_multimode
      ...
      end else if (!sha_en_i || hash_start) begin
        w_d = '0;
  ...
  assign w_index_d = (~sha_en_i || hash_start)  ? '0            :  // clear
  ...
  always_comb begin
    ...
    if (!sha_en_i || hash_start) sha_st_d  = ShaIdle;
  end
  ...
  // Idle
  assign idle_o = (fifo_st_q == FifoIdle) && (sha_st_q == ShaIdle) && !hash_start;
endmodule : prim_sha2
