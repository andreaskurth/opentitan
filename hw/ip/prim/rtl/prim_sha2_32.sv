...
#(
  ...
  if (MultimodeEn) begin : gen_multimode_logic
    ...
    always_comb begin : multimode_combinational
      ...
      // assign word_buffer
      if (!sha_en_i || hash_start) word_buffer_d = 0;
      ...
      if ((word_part_reset || hash_start || !sha_en_i)) begin
        word_part_count_d = '0;
      ...
      if (hash_start)       digest_mode_flag_d = digest_mode_i;      // latch in configured mode
      else if (hash_done_o) digest_mode_flag_d = None;               // clear
      else                  digest_mode_flag_d = digest_mode_flag_q; // keep

      // assign process_flag
      if (!sha_en_i || hash_start) process_flag_d = 1'b0;
