...
#(
  ...
  // A request gets denied if and only if
  // (1) the request is valid and
  // (2) no range matches it at all or the highest priority matching range denies it and
  // (3) range checks are not bypassed.
  assign range_check_fail = ctn_tl_h2d_i.a_valid &
                            (~(|addr_hit) | (deny_mask > grant_mask)) &
